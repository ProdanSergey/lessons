...
const mapDocumentToDomainModel = (
...
const mapDomainModelToDocument = (
...
export class MongoDBAppointmentRepository implements AppointmentRepository {
  ...
  async findMany({
    completed,
    limit = 100,
  }: FindManyFilter): Promise<Appointment[]> {
    ...
    const documents = await this.appointmentEntity
      .find(queryFilter)
      .limit(limit)
      .exec();
  ...
  async update(appointment: Appointment): Promise<Appointment> {
    const document = await this.appointmentEntity
