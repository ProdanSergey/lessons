...
export const getAppointment = async ({ id }: GetAppointmentParams) => {
  return get<Appointment>(
    `${CREATE_APPOINTMENT_PREFIX}/${encodeURIComponent(id)}`
  );
};
...
export const pickAppointment = async (
  { id }: PickAppointmentParams,
  { fullName, email }: PickAppointmentBody
) => {
  return post<undefined>(
    `${CREATE_APPOINTMENT_PREFIX}/${encodeURIComponent(id)}/pick`,
    { fullName, email }
  );
};
...
export const completeAppointment = async ({
  ...
  return post<undefined>(
    `${CREATE_APPOINTMENT_PREFIX}/${encodeURIComponent(id)}/complete`
  );
};
...
export const getAppointments = async ({ completed }: GetAppointmentsQuery) => {
  return get<Appointment[]>(
    `${CREATE_APPOINTMENT_PREFIX}?completed=${completed}`
  );
};
