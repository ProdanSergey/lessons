import {
  AppointmentDocument,
  AppointmentEntity,
} from "../../infra/mongo/entities/appointment";
...
const mapAppointmentToDocument = (
  appointment: Appointment
): { _id: Types.ObjectId } & AppointmentDocument => {
...
const mapDocumentToAppointment = (
  document: Document<Types.ObjectId, unknown, AppointmentDocument>
): Appointment => {
...
export class MongoDBAppointmentRepository implements AppointmentRepository {
  async create(appointment: Appointment): Promise<Appointment> {
    const record = await AppointmentEntity.create(
      mapAppointmentToDocument(appointment)
    );
  ...
  async update(appointment: Appointment): Promise<Appointment> {
    const record = await AppointmentEntity.findByIdAndUpdate(
      appointment.id,
      mapAppointmentToDocument(appointment),
      {
        new: true,
