...
export class AppointmentController extends BaseController {
  constructor(private readonly appointmentRepository: AppointmentRepository) {
    ...
    this.router.post(
      "/",
      route(this.handleCreate, appointmentCreateBodySchema)
    );
    this.router.delete("/:id", route(this.handleDelete));
    this.router.post(
      "/:id/pick",
      route(this.handlePick, appointmentPickBodySchema)
    );
  ...
  handleCreate = async (
    ...
    const response = await new CreateAppointmentCommand(
  ...
  handleGet = async (req: Request<AppointmentGetParams, AppointmentRecord>) => {
    ...
    const response = await new GetAppointmentCommand(
      this.appointmentRepository
    ).execute({ id });
  ...
  handleDelete = async (
    req: Request<AppointmentDeleteParams>,
    res: Response
  ) => {
    const { id } = req.params;
  ...
  handleList = async (
    ...
    const response = await new ListAppointmentCommand(
      this.appointmentRepository
  ...
  handlePick = async (
    req: Request<AppointmentPickParams, unknown, AppointmentPickBody>
  ) => {
    const { id } = req.params;
    const { fullName, email } = req.body;
